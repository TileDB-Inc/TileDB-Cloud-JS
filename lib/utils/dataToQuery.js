...
const createAttributeBufferHeaders = (attributes, bufferSize) => {
    ...
    const attributeBufferHeaders = attributes.map((attr) => {
        ...
        const isVarLength = isAttributeVarLength_1.default(attr);
        const isNullable = isAttributeNullable_1.default(attr);
...
const getMaxByteSizeOfAttribute = (attribute) => {
    const isVarLength = isAttributeVarLength_1.default(attribute);
    const isNullable = isAttributeNullable_1.default(attribute);
