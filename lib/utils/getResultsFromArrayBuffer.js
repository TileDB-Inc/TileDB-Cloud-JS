 ...
 */
const getResultsFromArrayBuffer = (arrayBuffer, attributeBufferHeaders, attributesSchema, options = {}) => {
    const data = {};
    /**
    ...
    attributeBufferHeaders.reverse().reduce((offset, attribute) => {
        ...
        if (isNullable && !options.ignoreNullables) {
            ...
            result = setNullables_1.default(convertToArray_1.default(result), nullablesArray, offsets);
        }
        // If result is a String slice the String by the offsets to make it an array
        if (isVarLengthSized &&
            !options.ignoreOffsets &&
            typeof result === "string") {
            result = groupValuesByOffsetBytes_1.default(convertToArray_1.default(result), offsets).map((s) => s.join(""));
        }
